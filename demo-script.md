...
| **ZK proof generation** | ✅ Real (local) / 🟡 Pre-generated (Vercel) | Locally: `nargo execute` + `bb prove` — real UltraHonk proof, real Barretenberg binary. On Vercel: proof was pre-generated by `npm run generate-proofs` and committed to the repo. The proof bytes are identical. |
...
**Architecture**: The frontend + reason/execute/audit APIs run on **Vercel free tier**. ZK proofs are pre-generated via `npm run generate-proofs` (one command) and committed to the repo. Any dev can clone and regenerate them.
