    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border/50 bg-surface/80 backdrop-blur-xl">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <Link href="/" className="text-lg font-semibold tracking-tight">
          <span className="text-accent">M</span>inima
        </Link>
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 text-center">
        ...
        <p className="scroll-hidden text-base sm:text-lg text-muted leading-relaxed max-w-2xl mx-auto mb-8 text-balance">
          Minima decides the minimum data a request actually needs, proves it with zero-knowledge,
          and acts on your behalf — without ever exposing the underlying facts.
        </p>
        ...
        <div className="scroll-hidden flex flex-wrap justify-center gap-3 mb-10">
          {[
            ...
            <span
              ...
              style={{
                color: color === "success" ? "#22c55e" : color === "accent" ? "#2dd4bf" : "#f59e0b",
                borderColor: color === "success" ? "rgba(34,197,94,0.2)" : color === "accent" ? "rgba(45,212,191,0.2)" : "rgba(245,158,11,0.2)",
                background: color === "success" ? "rgba(34,197,94,0.05)" : color === "accent" ? "rgba(45,212,191,0.05)" : "rgba(245,158,11,0.05)",
              }}
            >
              <span
                className="w-1.5 h-1.5 rounded-full animate-pulse-slow"
                style={{
                  background: color === "success" ? "#22c55e" : color === "accent" ? "#2dd4bf" : "#f59e0b",
                  boxShadow: color === "success" ? "0 0 6px rgba(34,197,94,0.6)" : color === "accent" ? "0 0 6px rgba(45,212,191,0.6)" : "0 0 6px rgba(245,158,11,0.6)",
                }}
              />
              {label}
            </span>
          ...
          {capabilities.map((c, i) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <span className="text-4xl font-bold text-white/10 font-mono">{c.num}</span>
                <span
          ...
          {steps.map((s) => (
            <div key={s.num} className="scroll-hidden relative">
              <div className="w-14 h-14 rounded-full bg-card border border-border flex items-center justify-center mb-5">
                <span className="text-accent font-mono font-semibold">0{s.num}</span>
              </div>
...
const components = [
  { name: "Next.js orchestrator", status: "live", desc: "Agent orchestration & UI" },
  { name: "BTL runtime", status: "live", desc: "Reasoning & disclosure decisions" },
  { name: "Noir proof engine", status: "live", desc: "ZK circuit compilation & proving" },
  { name: "ERC-4337 smart account", status: "live", desc: "Session keys & scoped execution" },
  { name: "Base Sepolia", status: "live", desc: "Testnet settlement & verification" },
];

function Architecture() {
  return (
    <section id="architecture" className="py-24 md:py-32 border-t border-border/50">
      <div className="max-w-6xl mx-auto px-6">
        <div className="scroll-hidden mb-16">
          <span className="text-xs font-mono text-accent tracking-widest uppercase">Infrastructure</span>
          <h2 className="text-3xl md:text-4xl font-semibold tracking-tight mt-3">Architecture</h2>
        </div>

        <div className="scroll-hidden bg-card border border-border rounded-2xl p-6 md:p-10">
          <div className="flex flex-col md:flex-row items-stretch gap-4 md:gap-0">
            {components.map((c, i) => (
              <div key={c.name} className="flex-1 flex flex-col items-center text-center relative">
                <div className="w-10 h-10 rounded-full bg-surface border border-border flex items-center justify-center mb-3">
                  <span className={`w-2 h-2 rounded-full ${c.status === "live" ? "bg-success" : "bg-muted"} glow-dot`} />
                </div>
                <span className="text-xs font-semibold mb-1">{c.name}</span>
                <span className="text-[11px] text-muted font-mono">{c.desc}</span>
                {i < components.length - 1 && (
                  <div className="hidden md:block absolute top-5 -right-2 text-muted/30 text-lg">→</div>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-3 gap-8 md:gap-16">
          {[
            ...
            <div key={s.label} className="scroll-hidden text-center">
              <div className="text-3xl md:text-5xl font-semibold text-accent mb-2 font-mono">{s.num}</div>
    ...
    <section id="security" className="py-24 md:py-32 border-t border-border/50">
      <div className="max-w-6xl mx-auto px-6">
        <div className="scroll-hidden max-w-3xl">
          <span className="text-xs font-mono text-accent tracking-widest uppercase">Security</span>
          ...
          <div className="space-y-4 text-base text-muted leading-relaxed">
            <p>
              Every claim is proven by a Noir circuit before any action executes. The proof is
              verified on-chain by the <ExternalLink href={`https://sepolia.basescan.org/address/${process.env.NEXT_PUBLIC_VERIFIER_ADDRESS || "0x87c90a98fA6b5bcA9a09d0FaE5fB818CCd7b6f8C"}`}>
              UltraHonk verifier contract</ExternalLink> — deployed and live on Base Sepolia.
            </p>
            <p>
              The session key is scoped by the <ExternalLink href={`https://sepolia.basescan.org/address/${process.env.NEXT_PUBLIC_ACCOUNT_ADDRESS || "0x4A050e97bBED792b91f72551EE09C18C94FECF92"}`}>
              AgentAccount smart contract</ExternalLink> to a single target and a limited action set.
              Minima can never move funds, transfer ownership, or act outside its permission slip.
            </p>
          </div>
          ...
          <div className="mt-10 p-4 border border-amber-500/20 bg-amber-500/5 rounded-xl">
            <p className="text-xs font-mono text-amber-400/80 leading-relaxed">
              ⚠ Testnet only. Experimental prototype — not audited. Deployed on Base Sepolia.
              Smart contracts and circuits are provided as-is for demonstration purposes.
            </p>
          </div>
        </div>
      </div>
    ...
    <section className="py-24 md:py-32 border-t border-border/50">
      <div className="max-w-6xl mx-auto px-6">
        <div className="scroll-hidden mb-12">
          <h2 className="text-2xl md:text-3xl font-semibold tracking-tight mb-3">Read the code</h2>
          <p className="text-muted text-sm">
            Everything is open source. Inspect the circuits, the contracts, and the agent.
          </p>
        </div>
          ...
          {repos.map((r) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <svg className="w-4 h-4 text-muted group-hover:text-white transition-colors" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6 text-xs text-muted">
            <a href="#capabilities" className="hover:text-white transition-colors">Features</a>
            <a href="#how-it-works" className="hover:text-white transition-colors">How it works</a>
            <a href="https://github.com/Saber1Y/Agent-Minima" target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors">GitHub</a>
          </div>
